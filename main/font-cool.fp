...
void main_default()
{
    ...
    lowp vec4 color = var_face_color;
    if (var_ultra_cool > 0.0) {
        color = vec4(1.0, 0.0, 0.0, 1.0);
    }
    gl_FragColor = mix(mix(color, var_outline_color, borders.y), vec4(0), borders.x);
}
